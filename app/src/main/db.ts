...
export function getSituazioneGlobale() {
  ...
  const entrateDirette = db!
    .prepare(
      `SELECT SUM(prezzo_unitario) as total FROM acquisti WHERE tipo = 'importo_cassa'`
    )
    .get() as any;
  ...
  const totaleUscite =
    ...
    (acconti.total || 0);
  const reale = (entrate.total || 0) + (entrateDirette.total || 0) - totaleUscite;

  // Detail for Vincolato Breakdown
  const dettagli = db!
    .prepare(
      `SELECT 
          a.nome_acquisto,
          COALESCE(SUM(q.importo_versato), 0) as raccolto,
          COALESCE(a.acconto_fornitore, 0) as acconto
       FROM acquisti a
       LEFT JOIN quote_membri q ON q.acquisto_id = a.id
       WHERE a.completato = 0 AND a.tipo IN ('acquisto', 'raccolta_fondo')
       GROUP BY a.id`
    )
    .all() as any[];

  const dettaglioVincolato = dettagli.map((d) => ({
    purchase_name: d.nome_acquisto,
    raccolto: d.raccolto,
    acconto_speso: d.acconto,
    vincolato_residuo: Math.max(0, d.raccolto - d.acconto),
  })); // Show ALL open purchases, even if vincolato is 0 (fully spent on advance)

  // Note: 'vincolati' total logic remains same, but we can verify it against detallio
  // For consistency with previous simple sum:
  const vincolatoNetto = Math.max(0, (vincolati.total || 0) - (acconti.total || 0));

  return {
    fondo_cassa_reale: reale,
    fondi_vincolati: vincolatoNetto,
    disponibile_effettivo: reale - vincolatoNetto,
    dettaglio_vincolato: dettaglioVincolato,
  };
}
...
export function getMovimentiFondo() {
  ...
  return db!
    ...
    SELECT * FROM (
        ...
        SELECT 'F-' || id as id, importo, descrizione, data, hidden FROM fondo_cassa
        ...
        -- 5. Importo Fondo Cassa (Entrata Diretta)
        SELECT 'I-' || a.id, a.prezzo_unitario, 'ENTRATA: ' || a.nome_acquisto, COALESCE(a.data_chiusura, a.data_creazione), hidden
        FROM acquisti a WHERE a.tipo = 'importo_cassa'
...
export function createAcquisto(
  ...
  if (!db) initDB();
  const trx = db!.transaction(() => {
    const isCompleted = tipo === "spesa_fondo" || tipo === "storico" || tipo === "importo_cassa" ? 1 : 0;
    ...
    if (tipo !== "spesa_fondo" && tipo !== "storico" && tipo !== "importo_cassa") {
...
export function updateAcquisto(
  ...
  const trx = db!.transaction(() => {
    db!
      ...
      .run(n, p, acconto, id);
    if (date) {
      const current = db!.prepare("SELECT data_creazione FROM acquisti WHERE id = ?").get(id) as any;
      const currentStr = current?.data_creazione || "";
      const currentDay = currentStr.split("T")[0];
...
export function hardResetDB() {
  if (!db) initDB();
  try {
    const backupName = performBackup(); // Safety backup
    const tables = ["quote_membri", "acquisti", "membri", "fondo_cassa"];
    db!.transaction(() => {
